// W51 — the one shared "who's currently speaking" registry, mirroring menu-registry.svelte.ts's
// singleton shape: starting a new utterance stops whatever else was speaking, and clicking the
// currently-speaking bubble's own button again stops it (toggle), the same "only one thing active"
// convention this app already uses for popovers.
//
...
export const speechRegistry = {
  ...
  speak(id: string, text: string): void {
    ...
    // (W76 — this comment previously claimed the opposite, that cancel() fires onend SYNCHRONOUSLY
    // and that this is what clears the old id. Were that true the toggle would be dead code and a
    // second click would re-speak. See tests/unit/speech-registry.test.ts.)
    s.cancel();
