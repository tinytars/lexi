<script lang="ts">
  ...
  // M98 Phase 2's hoisted ".btn"/".btn.primary" rule, now the component itself rather than a
  // class-name convention. `class` carries a caller's own local variant (`.done`, `.del`,
  // `.med-act`, …) alongside the base classes this renders — those variants' rule bodies stay
  // local to the caller, targeted with `:global()` since the element now lives in this component's
