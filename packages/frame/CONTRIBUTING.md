# Contributing to `@tinytars/frame`
...
outlast the first second consumer. See `ARCHITECTURE.md`'s "Session/auth controllers" section
before proposing a different pattern.
...
one (a new capability, a breaking API change). A [`CHANGELOG.md`](../../CHANGELOG.md) entry is the
same kind of deliberate, human-written call — not every patch bump gets one, only a release worth
explaining.

## Reporting a security issue instead of filing a PR

See the repo root [`SECURITY.md`](../../SECURITY.md) — vulnerabilities go through GitHub's private
vulnerability reporting, not a public issue or PR, until triaged.
