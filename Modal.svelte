<script lang="ts">
  // W23 — generic modal overlay (backdrop + close), reused to host Import / Export from the header.
  //
  // W70 — rebuilt on the native <dialog> element, which supplies as BROWSER PRIMITIVES what this
  // component previously did not have at all: a focus trap, focus restore on close, an inert
  // background, Escape handling, and top-layer stacking. Hand-rolling a focus trap is ~80 lines of the
  // most-often-wrong code in front-end and still yields neither top-layer nor `inert`.
  //
  // What was broken before, across all 15 call sites:
  ...
  // `wide` / `children` behave exactly as before, so ~10 of the 15 call sites change zero characters.
...
<style>
  ...
  /* M73 Phase 3 — the desktop margin is dead space on a short phone viewport. */
