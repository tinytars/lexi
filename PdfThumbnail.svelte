<script lang="ts">
  ...
  // W46 Phase 5 — a report's first-page thumbnail on the Reports list (HealthReports.svelte),
  // the ER-doctor path: see the first page -> click to scroll through the rest -> Download, no
  // menu digging. Loads pdfjs (via the shared pdf-render.ts) only once the card actually scrolls
  // into view (IntersectionObserver), so a Labs tab with many reports doesn't pay pdfjs's ~500 KB
  // cost until a thumbnail is actually about to render.
  interface Props {
    url: string;
    onOpen?: () => void;
  }
