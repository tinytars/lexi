...
const ctx = async (path: string[], auth?: "valid" | "bogus", method = "GET") => {
  ...
  return {
    ...
    env: makeEnv({ "dev/raw/pablo/echo-597cd4e7.pdf": RAW }),
    params: { path },
  };
};
...
describe("GET /api/raw/{id}/{file}", () => {
  it("401s without a bearer and with a wrong bearer", async () => {
    expect((await onRequestGet(await ctx(["pablo", "echo-597cd4e7.pdf"]))).status).toBe(401);
    expect((await onRequestGet(await ctx(["pablo", "echo-597cd4e7.pdf"], "bogus"))).status).toBe(401);
  });

  it("400s on a missing file segment or path traversal", async () => {
    expect((await onRequestGet(await ctx(["pablo"], "valid"))).status).toBe(400);
    expect((await onRequestGet(await ctx(["pablo", ".."], "valid"))).status).toBe(400);
  });

  it("404s when the object is absent", async () => {
    expect((await onRequestGet(await ctx(["pablo", "ghost.pdf"], "valid"))).status).toBe(404);
  });

  it("200s and streams the bytes with a pdf content-type, from the store-prefixed key", async () => {
    const res = await onRequestGet(await ctx(["pablo", "echo-597cd4e7.pdf"], "valid"));
    expect(res.status).toBe(200);
    expect(res.headers.get("content-type")).toBe("application/pdf");
    expect(await bytesOf(res)).toEqual(RAW);
  });

  it("lowercases the id so a capitalized vault key (e.g. 'Pablo') resolves the lowercase raw key", async () => {
    const res = await onRequestGet(await ctx(["Pablo", "echo-597cd4e7.pdf"], "valid"));
    expect(res.status).toBe(200);
    expect(await bytesOf(res)).toEqual(RAW);
  });
  ...
  it.each([
    ...
    const context = await ctx(["pablo", file], "valid");
    context.env = makeEnv({ [`dev/raw/pablo/${file}`]: RAW });
...
describe("DELETE /api/raw/{id}/{file} (W13h web-delete shape)", () => {
  it("401s without a bearer", async () => {
    expect((await onRequestDelete(await ctx(["pablo", "echo-597cd4e7.pdf"], undefined, "DELETE"))).status).toBe(401);
  });

  it("400s on a missing file segment or traversal", async () => {
    expect((await onRequestDelete(await ctx(["pablo"], "valid", "DELETE"))).status).toBe(400);
    expect((await onRequestDelete(await ctx(["pablo", ".."], "valid", "DELETE"))).status).toBe(400);
  });
  ...
  it("200s and removes the object so a subsequent GET 404s", async () => {
    ...
    const env = makeEnv({ "dev/raw/pablo/echo-597cd4e7.pdf": RAW });
    const c = async (method: string) => ({
      request: new Request("http://x/api/raw/pablo/echo-597cd4e7.pdf", { method, headers: await cookieHeader("valid") }),
      env,
      params: { path: ["pablo", "echo-597cd4e7.pdf"] },
    });
  ...
  it("is idempotent — deleting an absent object still 200s", async () => {
    expect((await onRequestDelete(await ctx(["pablo", "ghost.pdf"], "valid", "DELETE"))).status).toBe(200);
  });
});
...
describe("PUT /api/raw/{id}/{file} (W15/1 web upload)", () => {
  ...
  it("401s without / with a wrong bearer and stores nothing", async () => {
    const env = makeEnv();
    expect((await onRequestPut(await putCtx(["pablo", "x.pdf"], { env }))).status).toBe(401);
    expect((await onRequestPut(await putCtx(["pablo", "x.pdf"], { auth: "bogus", env }))).status).toBe(401);
    expect(env.store.size).toBe(0);
  });

  it("400s on a missing file segment or path traversal", async () => {
    expect((await onRequestPut(await putCtx(["pablo"], { auth: "valid" }))).status).toBe(400);
    expect((await onRequestPut(await putCtx(["pablo", ".."], { auth: "valid" }))).status).toBe(400);
  });

  it("400s on an empty body", async () => {
    expect((await onRequestPut(await putCtx(["pablo", "x.pdf"], { auth: "valid", body: new Uint8Array(0) }))).status).toBe(400);
  });

  it("413s on an oversized body and stores nothing", async () => {
    const env = makeEnv();
    const res = await onRequestPut(await putCtx(["pablo", "x.pdf"], { auth: "valid", body: new Uint8Array(24 * 1024 * 1024 + 1), env }));
  ...
  it("204s, stores under the lowercased store-prefixed key, and round-trips via GET", async () => {
    const env = makeEnv();
    const put = await onRequestPut(await putCtx(["Pablo", "2021October15-imaging-coronary-13da11c4.pdf"], { auth: "valid", body: RAW, env }));
    expect(put.status).toBe(204);
    expect(env.store.get("dev/raw/pablo/2021October15-imaging-coronary-13da11c4.pdf")).toEqual(RAW);
    const get = await onRequestGet({
      request: new Request("http://x/api/raw/pablo/2021October15-imaging-coronary-13da11c4.pdf", { headers: await cookieHeader("valid") }),
      env,
      params: { path: ["pablo", "2021October15-imaging-coronary-13da11c4.pdf"] },
    });
...
describe("/api/raw logging is PHI-free", () => {
  ...
  it("logs route/status/id only — never the filename or bytes", async () => {
    ...
    await onRequestGet(await ctx(["pablo", "echo-597cd4e7.pdf"], "valid"));
    ...
    expect(entry).toMatchObject({ route: "/api/raw", status: 200, id: "pablo", access: "owner" });
