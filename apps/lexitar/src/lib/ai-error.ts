// One typed error and ONE sentence per failure mode, shared by every model-backed Translate.
//
// The relay already classifies properly (insufficient_credit / ai_busy / model_error / ...), but
...
export function describeAiError(err: unknown): string {
  ...
  if (status === 402) return AI_ERROR_MESSAGES.insufficient_credit;
  if (status === 503) return AI_ERROR_MESSAGES.ai_busy;
