// Dedicated parser for the six R2/D1-native ops (scripts/commands/r2-ops.ts). ingest-args.ts's
// 42-flag Args/parseArgs is dead code in this repo (no caller besides its own unit test — ingest.ts
// itself was excluded from the plover-code -> lexi port) and pulls in flags none of these six ops
...
export interface R2OpsArgs {
  ...
  marker?: string;
  allMarkers: boolean;
  name?: string; // sync-treatment-attachments's optional treatment-name filter
}
...
export function parseR2OpsArgs(argv: string[]): R2OpsArgs {
  ...
  return { op: op!, client, store, mode: resolveMode(modeFlag), dryRun, force, marker, allMarkers, name };
}
