...
// W84 — the persona picker persists to the account for real (D1), and a Kodi chat turn shows Kodi's
...
const KODI = "Your LDL came in at 142 mg/dL on 2026-07-21.";
...
test("the persona picked in the sidebar survives a reload, and Kodi's chat turn keeps Lexi's original", async ({ page }) => {
  await stubChatHistory(page);
  await page.route("**/api/chat", (route) => route.fulfill({ json: { kind: "answer", answer: LEXI } }));
  await page.route("**/api/persona-adapt", (route) => route.fulfill({ json: { kind: "adapted", persona: "kodi", text: KODI } }));
  await openSynthetic(page);

  await personaToggle(page).getByRole("button", { name: "Kodi", exact: true }).click();
  await expect(personaToggle(page).getByRole("button", { name: "Kodi", exact: true })).toHaveAttribute("aria-pressed", "true");
  await expect.poll(async () => (await (await page.request.get("/api/account/persona")).json()).persona).toBe("kodi");

  await page.reload();
  await page.waitForSelector(".chat-tab textarea", { timeout: 15_000 });
  await expect(personaToggle(page).getByRole("button", { name: "Kodi", exact: true })).toHaveAttribute("aria-pressed", "true");
  const composer = page.locator(".chat-tab textarea");
  await expect(composer).toHaveAttribute("placeholder", /^Ask Kodi…/);
  ...
  await expect(answer.locator(".persona-tag")).toHaveText("Kodi");
  await expect(answer.locator(".turn-text")).toHaveText(KODI);
