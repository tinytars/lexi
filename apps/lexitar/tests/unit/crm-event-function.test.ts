...
async function mkAccount(stage: LifecycleStage) {
  ...
  await createAccount(w.db, { id, displayName: "A", lifecycleStage: stage });
  return id;
}
...
describe("emitLifecycleEvent (the seam)", () => {
  ...
  it("returns null for an unknown account", async () => {
    expect(await emitLifecycleEvent(w.db, "nope", "signup")).toBeNull();
  });
});
...
describe("POST /api/crm/event (stub)", () => {
  ...
  it("rejects a billing-driven event (must come from the payment webhook, not a client)", async () => {
    ...
    expect((await getAccount(w.db, id))!.lifecycleStage).toBe("active"); // no self-promotion
  });
