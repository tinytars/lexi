...
import {
  ...
  TREATMENT_INFER_MAX_TOKENS,
  MAX_TREATMENT_TEXT_CHARS,
} from "../../src/lib/treatment-infer-config";
...
export async function onRequestPost(context: { request: Request; env: Env }): Promise<Response> {
  ...
  try {
    ...
    // Photos are vision work; text that is already text is not. See treatment-infer-config.ts.
    const model = images.length > 0 ? TREATMENT_IMAGE_MODEL : TREATMENT_TEXT_MODEL;
    ...
    const messagesByCode: Record<string, string> = {
      ...
      anthropic_error: "image inference backend error",
    };
    return finish(status, { error: messagesByCode[errorCode] ?? "image inference backend error", errorCode }, { errorCode });
  }
}
