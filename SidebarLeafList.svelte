<script lang="ts">
  // THE sidebar row. Every list of individual items in the sidebar renders through this component:
  // a group's expanded children (Markers/Notes/Study/Reports/…) and the Assistant's threads alike.
  //
  // It used to be one of three: SidebarGroupList drew group rows, this drew children, and
  // ChatThreadList drew a hand-rolled copy of both because — and only because — a thread row needs
  // per-row actions and a child row had none. That single missing capability bought a second All row
  // at a different font size, an un-indented child list, a different label colour, and a rail rule
  // that disagreed with the other two. Adding the actions here is what let ChatThreadList be deleted.
  //
...
<style>
  /* Same slot Markers/Treatment/Hypothesis's .group-list occupies below the sidebar's divider —
     hidden on the desktop icon rail, forced visible in the mobile drawer. The container class stays
     distinct from .group-list on purpose: e2e scopes `.sub-item` through it, and tests/e2e/_nav.ts
     documents the collisions that forced the split. */
